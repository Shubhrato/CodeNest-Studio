...
function WhatsAppGlyph({ className = "size-4.5" }) {
  return (
...
const sizes = {
  ...
  lg: "text-[0.95rem] px-5 py-3 gap-2.5",
};
...
export default function WhatsAppButton({
  ...
  return (
    <a
      ...
      className={`group relative inline-flex items-center justify-center rounded-full border border-[#25D366]/35 bg-[#25D366]/10 font-medium text-[#3ddc84] transition-all duration-300 hover:-translate-y-0.5 hover:border-[#25D366]/60 hover:bg-[#25D366]/16 hover:text-[#57e79a] ${
        sizes[size] || sizes.md
      } ${full ? "w-full" : ""} ${className}`}
    >
      <WhatsAppGlyph className={`${glyphSizes[size] || glyphSizes.md} shrink-0`} />
      <span>{label}</span>
    </a>
  );
}
